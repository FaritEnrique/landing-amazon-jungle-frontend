...
const buildSlideTitle = (slide: HeroSlide) => {
  const title = [slide.titleBefore, slide.titleHighlight, slide.titleAfter]
    .filter(Boolean)
    .join(" ")
    .trim();
...
const hasSlideTitle = (slide: HeroSlide) => {
  return Boolean(
    slide.titleBefore?.trim() ||
      slide.titleHighlight?.trim() ||
      slide.titleAfter?.trim()
  );
};

const hasSlideOverlayContent = (slide: HeroSlide) => {
  return Boolean(
    slide.eyebrow?.trim() || hasSlideTitle(slide) || slide.description?.trim()
  );
};
...
const HeroCarouselAdminPage = () => {
  ...
  const activeCount = useMemo(() => {
    return slides.filter((slide) => slide.isActive).length;
  }, [slides]);
  ...
  const loadSlides = async (q = search) => {
    ...
    try {
      const response = await listHeroSlides({
        q,
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!formData.altText.trim()) {
      toast.error("Ingrese el texto alternativo de la imagen");
      return;
    }


    if (!editingId && !formData.imageDataUrl) {
      toast.error("Seleccione una imagen para el nuevo slide");
      return;
    }
    ...
    try {
      const payload = {
        ...(formData.imageDataUrl ? { imageDataUrl: formData.imageDataUrl } : {}),
        altText: formData.altText,
        eyebrow: formData.eyebrow || null,
      ...
      if (editingId) {
        ...
        toast.success("Slide registrado correctamente");
      }
  ...
  const handleToggleStatus = async (slide: HeroSlide) => {
    try {
      ...
      const message =
        error instanceof Error
          ? error.message
          : "No se pudo cambiar el estado del slide";
  ...
  const handleDelete = async (slide: HeroSlide) => {
    const shouldDelete = window.confirm(
      `¿Eliminar el slide "${buildSlideTitle(slide)}"?`
    );
  ...
  const hasFormTitle = Boolean(
    formData.titleBefore.trim() ||
      formData.titleHighlight.trim() ||
      formData.titleAfter.trim()
  );
  const hasFormOverlayContent = Boolean(
    formData.eyebrow.trim() || hasFormTitle || formData.description.trim()
  );
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        <div className="flex flex-col gap-4 rounded-3xl border border-slate-200 bg-white p-5 shadow-xl dark:border-slate-800 dark:bg-slate-900 sm:p-6 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <div className="inline-flex items-center gap-1.5 rounded-full border border-amber-100 bg-amber-50 px-3 py-1 text-[10px] font-bold uppercase tracking-wide text-amber-900 dark:border-amber-900/40 dark:bg-amber-950/40 dark:text-amber-300">
              Hero administrable
            </div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-relaxed text-slate-500 dark:text-slate-400">
              Administra las imágenes de fondo y el contenido superpuesto del
              bloque superior de la landing. Las imágenes activas se muestran en
              orden ascendente.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,460px)_1fr]">
            ...
            <div className="mb-5 flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-xs leading-relaxed text-slate-500 dark:text-slate-400">
                  Use imágenes horizontales de mínimo 1600px de ancho. El
                  backend convertirá JPG, PNG o WEBP a WEBP optimizado.
                </p>
              </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-wider text-slate-600 dark:text-slate-400">
                  Imagen de fondo {editingId ? "(opcional al editar)" : ""}
                </label>
                ...
                <label className="flex cursor-pointer flex-col items-center justify-center gap-2 rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-4 py-6 text-center transition hover:border-emerald-700 hover:bg-emerald-50 dark:border-slate-700 dark:bg-slate-950 dark:hover:bg-emerald-950/30">
                  ...
                  <span className="text-[11px] text-slate-500 dark:text-slate-400">
                    JPG, PNG o WEBP. Máximo 5 MB. Se guardará como WEBP.
                  </span>
                  <input
                    type="file"
                ...
                {previewImageUrl && (
                  <div
                    className="relative min-h-56 overflow-hidden rounded-2xl bg-cover bg-center shadow-inner"
                    style={{
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-1.5 sm:col-span-2">
                  ...
                  <input
                    type="text"
                    required
                    placeholder="Ej. Vista del albergue junto al río Amazonas"
                ...
                <div className="space-y-1.5 sm:col-span-2">
                  <label className="text-[10px] font-black uppercase tracking-wider text-slate-600 dark:text-slate-400">
                    Título antes del resaltado (opcional)
                  </label>
                  <input
                    type="text"
                    className="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-xs text-slate-900 transition-colors focus:border-emerald-800 focus:outline-none dark:border-slate-800 dark:bg-slate-950 dark:text-slate-100 dark:focus:border-emerald-500"
                    ...
                    onChange={(event) =>
                      setFormData({ ...formData, titleBefore: event.target.value })
                    }
                    disabled={isSubmitting}
                  />
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      setFormData({ ...formData, titleAfter: event.target.value })
                    }
                    disabled={isSubmitting}
                  />
                </div>
          ...
          <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-xl dark:border-slate-800 dark:bg-slate-900 sm:p-6">
            ...
            >
              <input
                type="search"
                placeholder="Buscar por título, etiqueta o texto..."
            ...
            {slides.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-slate-300 p-8 text-center dark:border-slate-700">
                ...
                <p className="mt-2 text-xs text-slate-500 dark:text-slate-400">
                  Cree el primer slide para reemplazar el fondo temporal del hero.
                </p>
              </div>
            ) : (
              <div className="space-y-4">
                {slides.map((slide) => {
                  const showOverlayContent = hasSlideOverlayContent(slide);
                  const showTitle = hasSlideTitle(slide);
                  ...
                  >
                    <div
                      ...
                      style={{
                        backgroundImage: `url("${resolveHeroImageUrl(slide.imageUrl)}")`,
                    ...
                    >
                      <div className="relative p-5 text-slate-950 sm:p-6">
                        ...
                        {showOverlayContent && (
                          <>
                            {slide.eyebrow && (
                              <p className="mt-4 text-[11px] font-bold uppercase tracking-wide text-white/80">
                                {slide.eyebrow}
                              </p>
                            )}
                    ...
                    <div className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
                      <div className="min-w-0">
                        ...
                        <p className="mt-2 text-[10px] font-semibold uppercase tracking-wide text-slate-400">
                          Actualizado: {new Date(slide.updatedAt).toLocaleDateString("es-PE")}
                        </p>
                      </div>

                      <div className="flex shrink-0 flex-wrap gap-2">
                        <a
                          href={resolveHeroImageUrl(slide.imageUrl)}
                          target="_blank"
                          rel="noreferrer"
